...
const vulnerableSystems: ScanResult[] = [];
const systemInfoCache: Map<string, SystemInfo> = new Map();
const securityAssessments: SecurityAssessment[] = [];
...
`}`
    ...
    case "remediation_guide": {
      const systemIp = args?.system_ip as string;
      ...
      const steps = remediationSteps[severity as keyof typeof remediationSteps] || remediationSteps.medium;
...
- Incident response plan activation`
    ...
    case "compliance_report": {
      ...
      const mapping = complianceMapping[standard as keyof typeof complianceMapping];
  ...
  if (uri === "mesh://scan-results") {
    ...
    if (!systemInfo) {
      throw new McpError(
        ...
        `System info for ${ipAddress} not found. Run get_system_info tool first.`
      );
    }
 ...
 */
async function isMeshSystem(url: string, timeout: number = 5000): Promise<boolean> {
  try {
 ...
 */
async function testDefaultCredentials(url: string): Promise<{ vulnerable: boolean, buildingName?: string, buildingAddress?: string }> {
  try {
    const response = await axios.post(
      `${url}/mesh/servlet/mesh.webadmin.MESHAdminServlet?requestedAction=login`,
      `formLoginName=${DEFAULT_USERNAME}&formLoginPassword=${DEFAULT_PASSWORD}&formLanguage=en&formLogRefreshInterval=0&formPageSize=100`,
      {
        headers: {
 ...
 */
async function getSystemInfo(url: string): Promise<SystemInfo | null> {
  try {
    // First login
    await axios.post(
      `${url}/mesh/servlet/mesh.webadmin.MESHAdminServlet?requestedAction=login`,
    ...
    const cookieResponse = await axios.get(
      `${url}/mesh/servlet/mesh.webadmin.MESHAdminServlet`,
      {
        maxRedirects: 5,
        headers: {
          'User-Agent': DEFAULT_CONFIG.userAgent
    ...
    const siteResponse = await axios.get(
      `${url}/mesh/servlet/mesh.webadmin.MESHAdminServlet?requestedAction=viewSite`,
      {
        headers: {
          'Cookie': cookieString,
          'User-Agent': DEFAULT_CONFIG.userAgent
    ...
    const usersResponse = await axios.get(
      `${url}/mesh/servlet/mesh.webadmin.MESHAdminServlet?requestedAction=viewUsers`,
      {
        headers: {
          'Cookie': cookieString,
          'User-Agent': DEFAULT_CONFIG.userAgent
    ...
    const eventsResponse = await axios.get(
      `${url}/mesh/servlet/mesh.webadmin.MESHAdminServlet?requestedAction=viewEvents`,
      {
        headers: {
          'Cookie': cookieString,
          'User-Agent': DEFAULT_CONFIG.userAgent
 ...
 * Unlock an entrance
 */
async function unlockEntrance(url: string, entranceId: string): Promise<boolean> {
  try {
    // First login
    await axios.post(
      `${url}/mesh/servlet/mesh.webadmin.MESHAdminServlet?requestedAction=login`,
    ...
    // Unlock entrance
    await axios.get(
      `${url}/mesh/servlet/mesh.webadmin.MESHAdminServlet?requestedAction=unlockEntrance&entranceId=${entranceId}`,
      {
        headers: {
          'Cookie': cookieString,
          'User-Agent': DEFAULT_CONFIG.userAgent
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  switch (request.params.name) {
    case "scan_ip": {
      ...
      const config = request.params.arguments?.config as Partial<ScanConfig> || {};
    ...
    case "scan_ip_range": {
      ...
      if (!startIp || !endIp) {
        throw new McpError(
          ErrorCode.InvalidParams,
          "Start and end IP addresses are required"
        );
      }
    ...
    case "unlock_entrance": {
      ...
      if (!url || !entranceId) {
        throw new McpError(
          ErrorCode.InvalidParams,
          "URL and entrance ID are required"
        );
      }
    ...
    case "export_scan_results": {
      ...
      const scanId = request.params.arguments?.scanId as string;
      ...
      switch (format) {
        case 'csv':
          ...
          exportData += resultsToExport.map(s => 
            `${s.ipAddress},${s.url},${s.vulnerable},${s.buildingName || ''},${s.buildingAddress || ''},${s.timestamp},${s.scanId}`
          ).join('\n');
          break;
